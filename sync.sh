...
case "$MODE" in
  pull)
    ...
    #
    # --exclude='corpus/validation-results.json' and
    # --exclude='corpus/derived-checksums/' added 2026-07-20, same session
    # ADR-0007 introduced both artifacts, proactively -- before either had
    # ever actually been generated by a real WSL run, so this couldn't yet
    # have caused a real incident, but it's the identical bug shape as the
    # other four excludes above it: a WSL-generated file that a stale
